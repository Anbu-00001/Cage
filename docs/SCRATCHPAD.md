...
- 2026-09-25: **Cell F (7B closure + COMMIT RULE, decoys ON, seeds 101-105): solved 4/5 (0.80), found 5/5, fns 1/5** — RECOVERS the full no-decoy rate (cell E 0.80) WITH decoys present. Commit-confidence Δ=+0.40 [-0.15,+0.95], identical to decoy-removal Δ. ana bcdab5(commit>=3/5 p.50)→TRUE. **KEY UPDATE to §6: the decoy-induced found_not_submitted gap IS closable by a mechanism-targeted prompt** ("shape-match IS proof, stop cross-checking") — NOT by the generic submit_nudge (D, 0.40→0.40, mere force) and NOT requiring scale. Two routes to 0.80: remove decoys (E) or tell it the doubt is unfounded (F). Revised the earlier "frontier beyond prompting" framing. Fixed committed run_live_sweep.py sys.path shadowing (eval/statistics.py vs stdlib) so it runs both as script and -m. commit_confidence wired as ablatable config flag (+test). §6 updated (6-cell matrix + 4 deltas). All servers stopped, guest up. NEXT PHASE: consolidate findings synthesis.
- 2026-09-25: **CONSOLIDATION PHASE.** Added RESULTS.md §0 "Findings at a glance" — resume-grade exec summary: 3-rung decomposition (find=scale 7B 5/5 vs 3B 0/5; submit-instruction=closure prompt; commit-under-distractors=COMMIT RULE recovers 0.40→0.80), headline (dominant failure is committing not finding; closable by mechanism-targeted prompt not scale), honesty bar (n=5, decisive on mechanism / underpowered on rates). Updated intro caveat (single-seed→small-N). Aligned docs/POSITIONING.md §4: memory-channel + tok/s now [FACT] (was stale [EST]), joules-per-solve still pending; flagged polyglot-stack aspirational-vs-built mismatch (Rust=telemetry sampler not tokio orchestrator; orchestrator is Python). All non-compute. Guest still up; servers stopped. Git still deferred per user.
- 2026-09-25: **n=10 EXTENSION + honest correction.** Extended the two decisive 7B cells (baseline A commit=0, commit-rule F commit=1) to n=10 (seeds 106-110 pooled with 101-105). Refactored aggregate_sweep.py to POOL rows across batches + recompute Wilson inline. **Results regressed to the mean vs n=5:** baseline 0.40→0.50, commit-rule 0.80→0.70. **Commit effect HALVED: Δ+0.40→+0.20 [-0.22,+0.62], still NOT significant.** The n=5 "recovers full loss to 0.80 / two routes / matches decoy removal" was OPTIMISTIC — walked back in §0 and §6. ana c185aa(commit stays >=2x baseline p.60)→FALSE. **What survived n=10 and is now SIGNIFICANT: the SCALE effect** — 7B 0.50 (n=10) vs 3B 0.00 (n=5), Δ+0.50 [+0.19,+0.81], CI excludes 0; found-rate 1.00 vs 0.00 categorical. Revised framing: TWO-rung story (find=scale, SIGNIFICANT; submit=prompt scaffolds, all directional/non-sig at n<=10). Robust facts are mechanistic (found vs submit, 1.00 found-rate, closure touches only submission); submit-rung RATES stay directional pending >=30-seed run. Calibration lesson logged IN the doc. §0/§6/POSITIONING all corrected. cellA2/cellF2 JSONs in eval/sweep_data/. Servers stopped, guest up.
