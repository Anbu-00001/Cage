...
- **This repo is early — but the core hardware numbers are now measured (`[FACT]`, 2026-09).** Memory bandwidth (**42.2 GB/s → single-channel**) and decode tok/s (0.5B 39.6, 3B ~4.4, 7B ~4.0, best at 2 P-cores) are measured on this exact host; see [docs/RESULTS.md](RESULTS.md) §1. **Still `[EST]`/unmeasured:** joules-per-solve (RAPL udev pending) and the ≥30-seed capability rates. What exists today is a 6-cell sweep (two decisive 7B cells at n=10, rest n=5; RESULTS §0/§6): *mechanistically* decisive but *rate-underpowered*. Quote its **findings** — **scale governs *finding*** (7B 10/10 vs 3B 0/5, Δ+0.50 [+0.19,+0.81], the one *significant* result), while prompt scaffolds only *directionally* help *committing* under distractors (a 0.40 n=5 effect regressed to +0.20 at n=10, CI still spans 0) — **not** its rates, as capability numbers. Don't let any pitch/resume line quote an unmeasured number.
- **The polyglot stack as-built differs from the aspirational bullets below.** Real: Python agent-loop/orchestrator + eval; C++ STREAM bench; **Rust** = a zero-dep telemetry sampler (not tokio, not the orchestrator); eBPF observability scripts. Fix the §5 bullets before using them if the reader will inspect the repo.
